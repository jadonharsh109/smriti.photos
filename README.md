...
sitting on your Mac or PC into a proper library — timeline, search, people, places, trips,
moments, duplicates — with **everything computed on your own machine**.

No cloud. No account. Nothing uploaded, ever — and your original files are never moved or
modified. The two optional model packs — face recognition (~280 MB) and search (~219 MB) —
...
Point it at a folder and it does the rest. The model packs behind People (~280 MB) and
Search (~219 MB) download once, when you first use each — the only downloads Smriti ever
makes. Everything after that runs on your machine.
...
- **Search** — type what the photo *shows* — "sunset over the sea", "a qr code" — and it is
  ...
  indore in 2024"* is a person, a place and a year, and only the words left over go to the
  model. The query never leaves your machine.
- **People** — face detection and recognition running on your CPU (SCRFD + ArcFace via
  onnxruntime), clustered into people you can name, merge and hide — and you choose the
  photo each person is shown by.
- **Moments** — a trip turned into a minute you can watch: the sharpest photos, faces kept
  in frame, blinks and near-duplicates left out, crossfaded to music over a title card. The
  music is generated by the project itself — no third-party audio ships in the app — and the
  result is an ordinary `.mp4` you can save and send.
...
smriti models --search      # one-time search-model download (~219 MB) — enables Search
...
- The **model packs** behind People and Search are not bundled — each is
  downloaded on request. The InsightFace models are licensed by their authors
  for **non-commercial research use**, and Apple's MobileCLIP under Apple's own
  research model licence. Those restrictions are theirs and follow the models,
  not this repository. (The music behind Moments has no such caveat: it is
  generated by a script in this repository and carries the project's licence.)
