...
def face_crop_image(img, x: float, y: float, w: float, h: float):
    """The crop the People page draws for a face: a square around the box
    with headroom, LANCZOS-downscaled to FACE_CROP_DIM. Pure PIL, no I/O, so
    the face worker can call it on the image it has already decoded — which
    is where crops are made now. `ensure_face_crop` below is the fallback for
    faces indexed before that was true. Returns None when the box is
    degenerate."""
    from PIL import Image
...
def write_face_crop(face_id: int, webp: bytes) -> None:
    dest = face_crop_path(face_id)
    tmp = str(dest) + ".tmp"
...
def ensure_face_crop(face_row, abs_path: str | None) -> str | None:
    ...
    if img is None:
        return fallback
    try:
        crop = face_crop_image(img, float(face_row["x"]), float(face_row["y"]),
                               float(face_row["w"]), float(face_row["h"]))
