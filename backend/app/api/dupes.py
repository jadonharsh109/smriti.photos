...
def exact():
    ...
    rows = db.query(
        ...
        "WHERE status='active' AND content_hash IS NOT NULL AND locked=0 "
        "GROUP BY content_hash HAVING n > 1 ORDER BY n DESC LIMIT 500",
    )
    for r in rows:
        items = db.query(
            ITEM_FIELDS + "WHERE f.content_hash=? AND f.status='active' AND f.locked=0",
            (r["content_hash"],))
...
def near():
    ...
    for g in db.query("SELECT * FROM dupe_groups WHERE kind='near' ORDER BY id"):
        items = db.query(
            ...
            "WHERE dgi.group_id=? AND f.status='active' AND f.locked=0", (g["id"],),
        )
